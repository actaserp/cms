...
public class CmsEc21SendService {
    ...
    private long generateAndSend(String spjangcd, String targetDate) throws Exception {
        ...
        // 2. EC21 생성
        byte[] fileBytes = buildEc21File(spjangcd, billings);
    ...
    /** EC21 재시도 — ERROR 건을 PENDING으로 리셋 후 재전송 (스케줄러 호출) */
    ...
    public Map<String, Object> resendBilling(List<Long> billingIds) {
        List<Map<String, Object>> groups = sqlRunner.getRows(/* skip_tenant_check */
                "SELECT DISTINCT spjangcd, deduct_date FROM cms_billing WHERE id IN (:ids) AND status='PENDING' AND deduct_type='EC'",
        ...
        for (Map<String, Object> g : groups) {
            ...
            try {
                Map<String, Object> failedFile = sqlRunner.getRow(/* skip_tenant_check */
                        ...
                        WHERE spjangcd=:s AND target_date=CAST(:td AS DATE) AND file_type='EC_REQUEST' AND send_status='FAILED'
                ...
                if (failedFile != null) {
                    ...
                    log.info("[CmsEc21Resend] 기존 파일 재전송 성공: spjangcd={} file={}", spjangcd, fileName);
                } else {
                    generateAndSend(spjangcd, targetDate);
                    log.info("[CmsEc21Resend] 파일 새로 생성+전송: spjangcd={} targetDate={}", spjangcd, targetDate);
                }
                sent++;
            } catch (Exception e) {
                failed++;
    ...
    private void sftpSendWithApiCredential(byte[] fileBytes, String fileName, String targetDate) throws Exception {
        ...
        HttpRequest req = HttpRequest.newBuilder()
                .uri(URI.create(apiBaseUrl + "/biz/batch?file_type=EC21&transaction_date=" + targetDate))
    ...
    private byte[] buildEc21File(String spjangcd, List<Map<String, Object>> billings) throws IOException {
